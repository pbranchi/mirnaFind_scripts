...
def rankplot(rankFile=None, bordaFile=None, Nfeat=10, positions=20, outFile=None):
    ...
    for i,a in enumerate(ax):
        ...
        cnt_tot = np.sum(RANKING[:, :]==fid[i], axis=0)
        a.bar(range(positions), cnt, color='b')
        meanl = a.axvline(fpos[i], 0, color='r', label='mean pos')
        # compute median
        med_nlist = []
        for cn in range(len(cnt)):
            med_nlist.append(np.repeat(cn,cnt_tot[cn]).tolist())
        med_list = list(chain.from_iterable(med_nlist))
        med = np.median(med_list)
        medl = a.axvline(med, 0, color='g', label='median pos')
    ...
    plt.figlegend((meanl, medl), ('mean pos', 'median pos'), loc=1)
